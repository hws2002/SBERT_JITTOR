...
from model.sbert_model import SBERTJittor, SBERTWithClassification
...
def _resolve_pooling(args, checkpoint):
    if isinstance(checkpoint, dict) and "pooling" in checkpoint:
        return checkpoint["pooling"]
    if isinstance(checkpoint, dict) and "pooling_mode" in checkpoint:
        return checkpoint["pooling_mode"]
    return args.pooling
...
def _load_eval_model(args, checkpoint):
    ...
    pooling = _resolve_pooling(args, checkpoint)

    if "model_state" in checkpoint:
        state = checkpoint["model_state"]
        if any(k.startswith("sbert.") for k in state) or any(k.startswith("classifier.") for k in state):
            wrapper = SBERTWithClassification(
...
def main():
    ...
    model = _load_eval_model(args, checkpoint)
