...
def train(args):
    config = TrainConfig.from_args(args, "training_mr")
    logger.info("=" * 70)
    logger.info("SBERT MR Training")
    logger.info("=" * 70)
    ...
    logger.info("Preparing MR training data (cached tokenization)...")
    train_dataset = prepare_mr_dataset(
    ...
    dev_dataset = prepare_mr_dataset(
        data_dir=args.data_dir,
        split="validation",
        tokenizer=tokenizer,
        max_length=args.max_length,
    ...
    logger.info("\nFinal Evaluation on MR Test Split")
    test_dataset = prepare_mr_dataset(
        data_dir=args.data_dir,
        split="test",
        tokenizer=tokenizer,
        max_length=args.max_length,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train SBERTJittor on MR",
